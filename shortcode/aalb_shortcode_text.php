...
class Aalb_Shortcode_Text {
  ...
  public function render($atts) {
    try {
      ...
      $validated_asins = $this->get_validated_asins($shortcode_attributes['asin']);
      ...
      $asins = $this->format_asin($validated_asins);
      ...
      $impression_params = $this->tracking_api_helper->get_impression_params($validated_link_id, AALB_SHORTCODE_AMAZON_TEXT, $shortcode_attributes);
      ...
      return str_replace(array('[[CLICK_URL_PREFIX]]','[[UNIQUE_ID]]', '[[Amazon_Link_Text]]'), array($click_url, str_replace('.','-',$products_template_key), $link_text), $this->template_engine->render($products_template_key, $products_key, $validated_template, $url, $validated_marketplace));
    } catch (Exception $e) {
   ...
   */
  private function format_asin($asin) {
    return preg_replace('/[ ,]+/', '-', trim($asin));
  }
